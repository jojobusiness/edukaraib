...
// Statuts "en attente" possibles (élève dans un groupe)
...
export default function TeacherLessons() {
  const [pendingIndiv, setPendingIndiv] = useState([]);   // is_group:false, status: booked | pending_teacher
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(qAll, async (snap) => {
      ...
      // ✅ Inclure aussi les leçons individuelles en 'pending_teacher'
      const pendingInd = all.filter(
        (l) => !l.is_group && (l.status === 'booked' || l.status === 'pending_teacher')
      );

      // ——— PENDING GROUP (tout ce qui N’EST PAS accepté/confirmé est visible, y compris 'pending_teacher') ———
      const pendingG = [];
      ...
      // Noms nécessaires :
      const idSet = new Set();
      // noms pour demandes groupe (élèves à valider)
      pendingG.forEach((p) => idSet.add(p.studentId));
      // ✅ noms pour demandes individuelles (sinon l’ID s’affiche)
      pendingInd.forEach((l) => { if (l.student_id) idSet.add(l.student_id); });
  ...
  return (
    <DashboardLayout role="teacher">
      ...
      <div className="bg-white rounded-xl shadow border p-5 mb-8">
              ...
              {pendingIndiv.sort(bySlot).map((l) => (
                <li key={l.id} className="border rounded-lg px-3 py-2">
                  <div className="flex items-center gap-3">
                    ...
                    <PendingStatusBadge status={l.status} />
        ...
        {/* Groupes (par élève) — inclut 'pending_teacher' et tout statut non confirmé */}
