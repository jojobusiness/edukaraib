...
// ...imports et tout ton code au-dessus inchangé...
  ...
  // Aperçu hebdo (lecture seule) avec navigation par semaine
  function WeekAvailabilityPreview({ availability={}, bookedSlots=[], remainingBySlot={}, orderDays=['Lun','Mar','Mer','Jeu','Ven','Sam','Dim'] }) {
    // mêmes utilitaires
    const [monthCursor, setMonthCursor] = React.useState(() => { const d = new Date(); d.setDate(1); d.setHours(0,0,0,0); return d; });
    ...
    const remainingFor = (day, hour) => {
      const v = remainingBySlot?.[`${day}:${hour}`];
      return typeof v === 'number' ? v : null;
    };

    return (
      <div className="mt-4">
        {/* Mini-calendrier (même style) */}
        <div className="mb-2 bg-white rounded-xl border shadow-sm p-3">
          <div className="flex items-center justify-between mb-2">
            <button type="button" onClick={() => setMonthCursor(d => addMonths(d, -1))} className="px-2 py-1 text-sm rounded border hover:bg-gray-50">‹</button>
            <div className="font-semibold">{monthCursor.toLocaleString('fr-FR', { month: 'long', year: 'numeric' })}</div>
            <button type="button" onClick={() => setMonthCursor(d => addMonths(d, 1))} className="px-2 py-1 text-sm rounded border hover:bg-gray-50">›</button>
          </div>
          <div className="grid grid-cols-8 gap-1 text-[11px]">
            <div className="text-gray-500 px-1"></div>
            {['Lun','Mar','Mer','Jeu','Ven','Sam','Dim'].map(h => <div key={h} className="text-center text-gray-500">{h}</div>)}
            {buildMonthMatrix(monthCursor).map((week,i)=>(
              <React.Fragment key={i}>
                <div className="text-center text-gray-500 font-mono px-1">S{isoWeek(week[0])}</div>
                {week.map((day,j)=>{
                  const isPast = (() => { const a = new Date(day); a.setHours(0,0,0,0); return a.getTime() < todayMidnight.getTime(); })();
                  ...
                  return (
                    <button
                      ...
                      title={isPast ? "Jour passé" : "Voir cette semaine"}
                    >
                      {day.getDate()}
                    </button>
                  );
          ...
          <table className="table-auto border text-xs">
            <thead>
              <tr>
                ...
                {hours.map(h => <th key={h} className="px-2 py-2 font-semibold text-slate-700">{h}h</th>)}
              </tr>
            </thead>
            <tbody>
              {orderDays.map((label, idx) => {
                ...
                return (
                  <tr key={label}>
                    ...
                    {hours.map(h => {
                      const booked = bookedMap.get(`${label}:${h}`) === true;
                      const dispo = Array.isArray(availability[label]) && availability[label].includes(h);
                      const locked = isSlotLockedByDate(dayDate, h);
                      const remaining = remainingFor(label, h);

                      let classes = 'relative w-7 h-7 rounded-md shadow flex items-center justify-center select-none ';
                      if (booked) classes += 'bg-red-500 text-white';
                      else if (locked) classes += 'bg-gray-300 text-white';
                      else if (dispo) classes += 'bg-green-500 text-white';
                      ...
                      return (
                        <td key={h} className="px-1 py-1">
                          <div className={classes} title={
                            booked ? 'Pris'
                            : locked ? 'Verrouillé (passé / <= +1h)'
                            : dispo ? 'Libre'
                            : 'Indispo'
                          }>
                            {remaining !== null && !booked && !locked && (
                              <span className="absolute -top-1 -right-1 min-w-[16px] h-[16px] px-1 rounded-full bg-indigo-50 text-indigo-700 border border-indigo-200 text-[10px] leading-[16px] text-center pointer-events-none">
                                {remaining}
                              </span>
                            )}
                          </div>
      ...
      <div className="relative bg-primary/5 border-b border-gray-100">
        <div className="max-w-5xl mx-auto px-4 py-10 md:py-14">
          ...
          <div className="mt-4 flex items-center gap-3">
            <WeekAvailabilityPreview
              availability={teacher.availability || {}}
              bookedSlots={bookedSlots}
              remainingBySlot={remainingBySlot}
              orderDays={['Lun','Mar','Mer','Jeu','Ven','Sam','Dim']}
            />
